...
from backend.utils import clear_proxy_env as _clear_proxy_env, money_to_yuan, to_float
...
def _money_to_yuan_local(value) -> float:
...
_market_holidays_cache: set[str] | None = None


def _load_market_holidays() -> set[str]:
    """从 data/market_holidays.json 加载休市日期，每年1月更新该文件即可。"""
    global _market_holidays_cache
    if _market_holidays_cache is not None:
        return _market_holidays_cache
    path = Path(BASE_DIR) / "data" / "market_holidays.json"
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
        _market_holidays_cache = set(data.get("holidays", []))
    except Exception:
        logger.warning("加载交易日历失败，使用内置2026年数据兜底")
        _market_holidays_cache = {
            "2026-01-01",
...
def is_trading_day() -> bool:
    ...
    节假日配置在 data/market_holidays.json，每年更新一次即可。
    """
    today = date.today()
    if today.weekday() >= 5:
        return False
    return today.strftime("%Y-%m-%d") not in _load_market_holidays()
...
def get_board_rankings(snapshot_type: str = "sector", limit: int = 20, live: bool = False) -> dict:
    ...
    if heat_df is not None and not heat_df.empty:
        for _, row in heat_df.iterrows():
            ...
            if not name:
                continue
            heat_map[name] = {
                "heat_score": to_float(row.get("板块异动总次数")),
                "main_net_inflow": _money_to_yuan_local(row.get("主力净流入")),
            }
    ...
    if flow_df is not None and not flow_df.empty:
        for _, row in flow_df.iterrows():
            ...
            net_inflow = _money_to_yuan_local(row.get("净额"))
            heat_score = heat.get("heat_score") or (
                abs(net_inflow) / 100000000 + abs(to_float(row.get("行业-涨跌幅"))) * 10 + to_float(row.get("公司家数"))
            )
            rows.append({
                "symbol": name,
                "name": name,
                ...
                "turnover": _money_to_yuan_local(row.get("流入资金")) + _money_to_yuan_local(row.get("流出资金")),
                "main_net_inflow": net_inflow,
                "capital_inflow": _money_to_yuan_local(row.get("流入资金")),
                "capital_outflow": _money_to_yuan_local(row.get("流出资金")),
                "company_count": int(to_float(row.get("公司家数"))),
                "leading_stock": str(row.get("领涨股", "")),
                "leading_stock_change_pct": to_float(row.get("领涨股-涨跌幅")),
                "heat_score": heat_score,
                "source": "akshare_live",
                "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            })
