...
| **P2.1 费率账本** | ✅ 已完成 | 费率表 + 每日计提（21:00）+ 年度账本 + 赎回费预估器（含"再等 N 天可省 ¥X"提示） |
| **P2.2 用户决策复盘** | ✅ 已完成 | 自动从 trades 生成决策快照 + 30 天回填胜率 + 行为偏差雷达图 + 月度报告 |
| **P2.3 OCR 持续闭环** | ✅ 已完成（首版） | 每周提醒 + OCR 快照差异预览 + 份额变化推断交易 + 用户确认后落库；缺失持仓仅告警不自动赎回 |
| **P2.4 老基民小工具集** | ✅ 已完成（首版） | 持有里程碑 / 季报披露 / 同公司转换 / 节假日提醒 / 费率体检 5 个工具，已接入工具箱页面 |

> **2026-05-31 验收标记（Codex）**：P0-P2 主线已完成自动化/副本验证和桌面烟测；原始库未被写入。剩余业务验收依赖外部输入：① 新鲜支付宝/天天基金截图；② 活跃持仓真实费率表。
...
#### 2026-05-27 Codex 首版落地

- 后端新增 `/api/ocr/status`、`/api/ocr/diff-preview`、`/api/ocr/confirm-snapshot`
- `ocr_reconcile_service_v3.py` 对比当前活跃持仓与本次 OCR 快照，推断买入/卖出交易，并跳过同日同份额重复 OCR 交易
- 前端 `OcrImport.vue` 加入同步提醒、差异刷新、推断交易预览、缺失持仓告警；`App.vue` 每日最多弹一次到期提醒
- 安全边界：本次截图缺失的现有持仓只提示，不自动生成“全部赎回”，避免分页截图漏传造成误操作
...
- 后端新增 `backend/services/senior_toolbox_service_v3.py` + `/api/toolbox/*`
- 前端新增 `ToolBox.vue`，并接入 `/toolbox` 路由和侧边栏入口
- 已上线 5 个零新增表工具：持有里程碑、季报披露、同公司转换、节假日提醒、费率体检
- 修复里程碑降费节省金额计算、节假日周末桥接识别、基金转换公司不一致提示，并补充专项测试
