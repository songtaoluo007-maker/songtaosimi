...
首次打开会要求创建本机所有者账号。账号密码只保存在本地 SQLite 数据库中，业务接口默认需要登录后才能访问。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\self_check.ps1
```

自检会使用独立临时数据库验证后端语法、前端构建、登录初始化、未登录拦截和静态资源缓存策略，不会污染你的正式持仓数据。
